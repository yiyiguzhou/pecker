...
from framework.exception.exception import TimeoutExpired


def get_sys_encoding_style():
    """获取系统编码方式， 只用window判断，Linux和MAC默认为utf-8
    returns:
        编发方式
    """
    coding_style = 'utf-8'
    ...
    if g_resource['system'] == const.SYSTEM_WINDOWS:
        coding_style_dic = {'936': 'gbk', '65001': 'utf-8'}

        p = subprocess.Popen("chcp", shell=True, stdin=subprocess.PIPE, stdout=subprocess.PIPE)
        ret_msg, ret_err = p.communicate(timeout=3)

        if isinstance(ret_msg, bytes):
            if b'936' in ret_msg:
                coding_style = coding_style_dic.get('936')
            elif b'65001' in ret_msg:
                coding_style = coding_style_dic.get('65001')
            g_framework_logger.info('system console coding style: {}'.format(ret_msg.decode(coding_style)))
...
class IpeckerPopen(subprocess.Popen):
    ...
    阻塞性线程
    """
    __coding_style = get_sys_encoding_style()
    ...
    def execute(self, input=None, timeout=None):
        ...
        try:
            ...
            if self.__except_flag:
                raise TimeoutExpired(self.__cmd, self.__timeout)
        try:
...
class IpeckerThread(object):
    """非阻塞性线程程处理类"""
    def __init__(self, cmd=None, std_flag=False, log_file=None, time_interval=0.1, repeat_flag=False, shell=False):
        self._log_file = None
        self._run_flag = True
        self._cmd = cmd
...
if __name__ == "__main__":
    ...
    p = IpeckerRepeatThread('adb shell "dumpsys meminfo |grep com.qiyi.video:plu"', log_file="performance.txt", time_interval=1)
    p.start()
    time.sleep(10)
    p.stop()

    # try:
    #     i = IpeckerPopen("tasklist | findstr node", shell=True, except_flag=True, timeout=30)
    #     out, err = i.execute()
    ...
    #
    # try:
    #     # p = IpeckerThread("adb logcat -v time", log_file_path="logcat.txt")
    #     p = IpeckerThread("ping 127.0.0.1 -n 30", log_file="logcat.txt")
    #     p1 = IpeckerThread("ping 127.0.0.1 -n 30", log_file="logcat1.txt")
    #     p.start()
    #     p1.start()
    #     time.sleep(5)
    #     p.stop()
