...
"""
File Name:      MyInfoSmallGameGoSmallGC
Author:         zhangwei04
...
class MyInfoSmallGameGoSmallGC(TestsuiteNormal):
    """
    预置条件：无
    """
    def setup(self):
        self.desc = "个人中心-小游戏-登录页"
    ...
    def test(self):
        ...
        assert_true(DUT.MyInfoPage.check_tab_ui(tab_name=self.data.tab_name), "检测小游戏tabUI", target=DUT)
        assert_true(DUT.MyInfoPage.small_game_tab_goto_small_game_center(), "小游戏tab进入小游戏中心", target=DUT)
        assert_true(DUT.SmallGamePage.check_ui(), "检测小游戏中心UI", target=DUT)
