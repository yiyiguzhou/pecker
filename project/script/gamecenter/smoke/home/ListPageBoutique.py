...
class ListPageBoutique(TestsuiteNormal):
    """
    用例描述：榜单页-精品榜tab
    预置条件：运营位：1469配置了多款游戏。
    """
    def setup(self):
        self.desc = "榜单页-精品榜tab"
        DUT.device.start_log()
        DUT.Common.reset_app()

    def test(self):
        g_logger.info(self.desc)
        g_logger.info("运营位：1469配置了三款游戏。")
        ...
        assert_true(DUT.Common.click_title(self.data.title), "进入精品榜tab", target=DUT)
