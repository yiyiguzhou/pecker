...
"""
File Name:      RoomSendBagBatchGift
Author:         zhangwei04
...
class RoomSendBagBatchGift(TestsuiteNormal):
    def setup(self):
        self.desc = "直播间-背包-批量道具赠送"
    ...
    def test(self):
        ...
        assert_true(DUT.LiveRoom.bag_send_gift(gift_index=self.data.gift_index, gift_name=self.data.gift_name,
                                               count=self.data.count, group=self.data.group),
                    desc="点击背包赠送礼物", target=DUT)
        assert_true(DUT.LiveRoom.check_chat_gift_info(account_section='user1', gift_name=self.data.gift_name,
                                                      gift_num=self.data.count, group=self.data.group),
