...
class BaseSwitchThird(TestsuiteNormal):
    """
        用例描述：基线切第三方登录
        预制条件：1.本地没有缓存的手机
                  2.基线无登录状态
    ...
    def test(self):
        ...
        assert_true(DUT.H5sdk.sidebar(self.data.is_tourist, self.data.is_sms, self.data.phone_num), "点击侧边栏切换账号进入第三方登录成功",target=DUT)
        assert_true(DUT.H5sdk.qq_login(), "QQ账号登录成功", target=DUT)
        # assert_true(DUT.H5sdk.phone_return(), "点击物理返回键成功退出游戏", target=DUT)
        # assert_true(DUT.H5sdk.into_h5game('浅浅女王梦H5'), "再次进入非棋牌游戏成功")
        # assert_true(DUT.H5sdk.account_display(self.data.text), "账号显示正确", target=DUT)

    def teardown(self):
        DUT1.device.stop_log()