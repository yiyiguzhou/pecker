...
class BaseSwitchSms(TestsuiteNormal):
    """
        用例描述：基线切短信登录
        预制条件：1.本地没有缓存的手机
                  2.基线登录状态
    ...
    def test(self):
        ...
        assert_true(DUT.H5sdk.sidebar(self.data.is_tourist,self.data.is_sms,self.data.phone_num), "点击侧边栏切换账号进入短信登录页面成功", target=DUT)
