...
class TouristSwitchThird(TestsuiteNormal):
    """
        用例描述：游客账号切换第三方登录
        预制条件：1.本地没有缓存的手机
                  2.基线无登录状态
    ...
    def test(self):
        ...
        # # assert_true(DUT.H5sdk.qq_login(self.data.qq_account,self.data.qq_password), "成功进入qq页面", target=DUT)
        # assert_true(DUT.H5sdk.phone_return(), "成功退出游戏", target=DUT)
        # assert_true(DUT.H5sdk.coin(), "再次进入游戏成功", target=DUT)
