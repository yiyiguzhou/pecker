...
class  Mainland(Android):
    ...
    def Account_correct(self,cache_account):
        ...
        try:
            ...
            time.sleep(5)
            self.device.find_element_by_xpath("//android.widget.TextView[@text='{}']".format(cache_account), timeout=5)
            time.sleep(2)
        except:
            return False
        return True
    ...
    def Other_login(self, vip_time):
        ...
        try:
            ...
            try:
                ...
                self.device.find_element_by_id(self.conf.into_logout.id, timeout=5).click()
                self.device.find_element_by_id(self.conf.logout_mobile.id, timeout=5).click()
                self.device.find_element_by_id(self.conf.other_log.id, timeout=5).click()
        except Exception as e:
            pass
            # 滑动查找切换账号
    ...
    def mobilelogin_sms(self, phone):
        ...
        try:
            self.device.find_element_by_id(self.conf.get_verifycode.id, timeout=10).click()
        ...
        try:
            # 进入游戏
            self.device.find_element_by_id(self.conf.into_game.id, timeout=5).click()
            time.sleep(2)
        except Exception as e:
            pass
            # 切回appium输入法
    ...
    def initialize_wetchat_login(self):
        ...
        """
        try:
            self.device.find_element_by_id(self.conf.init_wetchat.id, timeout=5).click()
            time.sleep(5)
        except Exception as e:
            return False
        return True
    ...
    def account_password_login(self, account, password):
        ...
        try:
            ...
            try:
                ...
                self.device.find_element_by_id(self.conf.initialize_qq.id, timeout=5).click()
            except Exception as e:
                pass
                self.device.find_element_by_id(self.conf.initialize_qq.id, timeout=5).click()
        except  Exception as e:
    ...
    def qq(self, qq, qq_password):
        ...
        try:
            ...
            self.device.find_element_by_xpath("//android.widget.Button[@text='授权并登录']", timeout=5).click()
            time.sleep(3)
        except:
            try:
                self.device.adb.adb_shell("input keyevent KEYCODE_MENU ")
    ...
    def wetchat_login(self,wechat_account,wechat_password):
        ...
        try:
            try:
                self.device.find_element_by_id(self.conf.into_wechat_login.id, timeout=5).click()
                time.sleep(3)
                self.device.find_element_by_id('com.iqiyigame.sdk:id/login_wechat', timeout=5).click()
            except Exception as e:
