...
def excel_to_xml(workbook_name, line_list):
    ...
    interface_test_path = os.path.join(path, "script")
    line_list = change_line_to_new_order(line_list)
    for ll in line_list:
        interface_test_path = os.path.join(interface_test_path, ll)
    for sheetname in sheetnames:
        ...
        file_name = os.path.join(interface_test_path, file_path)
        try:
            print(url_name_text)
            ...
            for row in range(sheet.max_row):
                if row == 0:
                    ...
                    expect = add_child(sheet["F" + "1"].value, expect_code, expect, False)
